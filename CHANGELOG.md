...
Two things this marketplace promised and could not do. `/investigation-coach:onboard --tour` said
it ran three skills; the harness blocked it from running any. And the notes it wrote were called an
Obsidian vault while the one feature that makes a vault worth opening — the graph — stayed empty.

### The documentation skills are model-invocable

`onboard`, `map`, `study` and `blueprint` lose `disable-model-invocation: true`. `--tour` now
actually invokes `map` and `study` through the Skill tool, and `/ai-coach:start --run` chains the
whole day-one sequence with the flags already worked out. The roster steps do not move: identity
and project declaration stay lines you type, because nobody registers you on your behalf.

That is a real cost, stated rather than buried — four descriptions now enter the model's context
every session where none did before, and the description is the only thing left gating a skill that
reads a lot of code and writes files. Each one says explicitly when it may fire and that it never
fires on its own.

`/start` also learned to look at the shape of the repo. More than one service manifest below the
root, more than one service in a compose file, or more than one repo in `.ai-coach/project.md`, and
it forwards `--project` — the flag a microservices team needs and has no way to know about.

### Frontmatter that Obsidian actually reads

Every note template put `> Generated by …` on line 1 and the `---` block after it. Obsidian parses
frontmatter only at byte 0, so every tag and alias these skills carefully wrote was discarded on
open. Frontmatter now comes first and the marker sits directly below it; the overwrite check looks
in the first 10 lines instead of only the first, so both orders stay safe while old vaults catch up.

### One note per component

`/investigation-coach:map` writes `docs/onboarding/components/<name>.md` for every service it
traces, and each traced edge becomes a wikilink in that note. The architecture is a link graph now,
so the graph view draws it without being told anything. The canvas follows: its nodes are `file`
nodes pointing at those notes rather than text cards nobody can open — a canvas edge was never
visible to the graph view, which is why the picture and the graph had to stop being the same claim.

`docs/.obsidian/graph.json` is written once, only when absent, and never overwritten: four colour
groups so a graph opens legible. It is the single sanctioned exception to "no skill generates
`.obsidian/`", and it is committed rather than ignored.

### One vault root, and the orphans closed

The vault root is `docs/`, said once and consistently — it was previously three different folders
in three files, so one set of links was always broken. Cross-directory wikilinks are vault-absolute
from there; the two `[[../note]]` forms Obsidian never resolved are gone. `docs/features/index.md`
now gets written by the skill whose hub has been linking it all along, `plan.md` gained the one
link that stops it orphaning, and the onboarding hub gained the Features and Components sections
that reach `map`'s output.

### `ENGINE team-list` works

It read the `trust` table from the per-project database. That table only ever existed in the user
database — the four other readers all knew that. Any roster with a member died on
`no such table: trust`. One word, and a test that spawns a real process, because an in-process
handle hid it.
