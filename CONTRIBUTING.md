...
So a command's product is a prepared runway, never a landing it performed itself: read state, check
the gates, hand over the exact line to type. A command also cannot tell what is installed by
looking at its own context — user-only skills are invisible to it by design — so it runs
`claude plugin list` instead. Both of these were learned by shipping the opposite.
