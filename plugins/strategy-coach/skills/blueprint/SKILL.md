...
0. **Scaffold the vault, then look at what is already in it.** This used to be `/strategy-coach:vault`,
   ...
   directories do not survive git); write `docs/home.md`, `docs/conventions.md` and the empty
   `docs/features/index.md` from `references/layout.md` — `home.md` links that index, so scaffolding
   the directory without it manufactures the broken link this step exists to prevent. Append
   `docs/.obsidian/workspace*` and `docs/.obsidian/cache/` to the repo-root `.gitignore` if
   missing, appending only, never rewriting that file. The paths are rooted at the vault on
   purpose: a pattern containing a slash is anchored to the directory its `.gitignore` sits in, so
   a bare `.obsidian/workspace*` at the repo root would never match anything under `docs/`. Never
   widen them either — `/investigation-coach:map` writes `docs/.obsidian/graph.json` and it is
   meant to be committed. Create nothing else under
   `.obsidian/` — Obsidian writes its own, and a hand-made file with the wrong schema version is
   worse than none.
...
- **Never overwrite a note without a `> Generated by /strategy-coach:` marker in its first 10
  lines.** The marker sits directly below the frontmatter block, because Obsidian only parses
  frontmatter that starts at byte 0 and a marker above it costs the note every tag it carries. No
  marker means a person wrote it: report it as skipped and move on. This covers the scaffolded hub
  and conventions page as much as the business notes.
