...
0. **Scaffold the vault, then look at what is already in it.** This used to be `/strategy-coach:vault`,
   ...
   `.obsidian/workspace*` and `.obsidian/cache` to `.gitignore` if missing, appending only, never
   rewriting that file, and never widen those patterns: `/investigation-coach:map` writes
   `docs/.obsidian/graph.json` and it is meant to be committed. Create nothing else under
