...
8. **Join the vault.** Append one row for this feature to `docs/features/index.md` — the hub
   `docs/home.md` already links, whose template is in `references/formats.md`. Create it if it is
   absent; amend it when it carries a `> Generated by /strategy-coach:` marker in its first 10
   lines; report it as skipped, untouched, when it does not. Nothing else in the vault reaches a
   spec, so this row is the difference between a specified feature and an orphan.
9. **Report and remember.** The two paths, the DoD row count, the open unknowns, and how to
