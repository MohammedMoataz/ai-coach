-- AI Coach user-scope schema. One database per machine at ~/.ai-coach/user.db, holding what
-- belongs to the person rather than to any one project. Idempotent: exec on every open.

-- Trust is PRIVATE and per-person, not per-project: you rate a teammate once and that judgment
-- applies in every project you share with them. It never leaves this machine, never appears in
-- .ai-coach/team.md, never travels in a seed. Nobody should have to commit "I do not trust this
-- teammate yet" to a repository.
...
-- Every project AI Coach has seen, and which directory holds its database. This is what makes
-- cross-project search possible at all once storage is one file per project.
CREATE TABLE IF NOT EXISTS projects (
  key     TEXT PRIMARY KEY,             -- the project identity (see .ai-coach/project.md)
