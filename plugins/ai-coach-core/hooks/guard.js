  ...
  try {
    ...
    if (tool === 'Read') {
      if (sensitivePath(String(input.file_path || ''))) {
        ask('ai-coach-guard: reading a credentials-type file — its content would enter the model context.');
      }
    } else if (OUTBOUND.has(tool)) {
      for (const re of ASK) {
        if (re.test(text)) ask('ai-coach-guard: possible secret in outbound payload — confirm before running.');
      }
      for (const tok of String(input.command || '').split(/[\s"'`;|&()<>=,]+/)) {
        if (tok && sensitivePath(tok)) {
          ask('ai-coach-guard: command touches a credentials-type file (' + tok + ') — confirm before running.');
        }
      }
    }
  } catch (err) {
