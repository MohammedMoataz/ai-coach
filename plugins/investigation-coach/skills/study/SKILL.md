---
description: Write study material explaining this project's patterns and technologies, the why rather than the how-to, into docs/study/. Use when the user explicitly asks ("/study", "help me study this codebase", "explain the patterns here"), or when onboard --tour or /ai-coach:start --run chains it; never proactively, it reads a lot of code and writes files.
...
found, not just the core ones. `--project`: span the repos in `.ai-coach/project.md`, writing into
the `docs/` of the repo you ran from — one vault, however many repos it explains, with each area
directory under its repo (`docs/study/<repo>/<area>/`). Two services that both have a `worker` area
are two areas, and an unqualified path would merge them into one file that is true of neither.
`cross-cutting/` stays at the top level: a pattern shared across repos is the one thing that is not
per-repo.

## Steps

1. **Detect** what exists under `docs/study/` (a `> Generated by /investigation-coach:study` line
   in the first 10 lines = regenerable; without it = hand-written, ask first). A `study/` directory
   at the repo root is output from before this path moved — say so and offer to move it; never
   write to both.
...
docs/study/
  ...
  <area>/*.md         one directory per discovered area, one file per pattern/technology
                      (`<repo>/<area>/*.md` under --project, so same-named areas stay apart)
