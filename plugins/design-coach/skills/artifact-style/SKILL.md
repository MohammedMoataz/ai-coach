...
3. **Build from `references/skeleton.html`.** It is the one copy of the base CSS (tokens in all
   ...
   zoom script. Copy it, set the title, replace the fallback hex with the brief's values — names
   stay — and delete what the page does not use. Never paste a second copy of the CSS from
   memory; the lint is written against the skeleton's names.
